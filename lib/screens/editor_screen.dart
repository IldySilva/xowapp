...
import '../services/web_video_export.dart';
...
class _EditorScreenState extends State<EditorScreen> {
  ...
  String get _videoPath => widget.video.pathOrUrl;

  @override
  void initState() {
    super.initState();
    _loadFrames();
    _controller = widget.video.createController()
      ..initialize().then((_) {
        setState(() {});
    ...
    return Scaffold(
      ...
      body: LayoutBuilder(
        builder: (context, constraints) {
          final minW = kIsWeb ? 640.0 : 800.0;
          final minH = kIsWeb ? 420.0 : 500.0;
          if (constraints.maxHeight < minH || constraints.maxWidth < minW) {
            return const Center(
              child: Text(
  ...
  Future<void> _exportVideo() async {
    ...
    if (kIsWeb) {
      await _exportVideoWeb();
      return;
    }

    setState(() => _isExporting = true);
    try {
      final workDir = await AppPaths.workDir();
      final stamp = DateTime.now().millisecondsSinceEpoch;
      final outputPath = '$workDir/Xowcase_Export_$stamp.mp4';
      final bgPath = '$workDir/xowcase_temp_bg.png';
      final maskPath = '$workDir/xowcase_temp_mask.png';
      final tempFramePath = '$workDir/xowcase_temp_frame.png';

      final durationSecs =
          (_controller.value.duration.inMicroseconds / 1000000.0)
              .toStringAsFixed(3);
      ...
      if (_resolution == '16:9 (YouTube)') {
        ...
        if (_selectedFramePath == 'frameless') {
          double videoW = _controller.value.size.width == 0
              ? 1920
              : _controller.value.size.width.toDouble();
          double videoH = _controller.value.size.height == 0
              ? 1080
      ...
      if (_backgroundGradient != null) {
        final rect =
            Rect.fromLTWH(0, 0, canvasW.toDouble(), canvasH.toDouble());
        final stops = [
          for (int i = 0; i < _backgroundGradient!.length; i++)
            i / (_backgroundGradient!.length - 1),
        ];
        final paint = Paint()
      ...
      if (_selectedFramePath == 'frameless') {
        final videoW = _controller.value.size.width == 0
            ? 1920
            : _controller.value.size.width.toDouble();
        final videoH = _controller.value.size.height == 0
            ? 1080
            : _controller.value.size.height.toDouble();

        double scale = 1.0;
        if (videoW > canvasW * (1.0 - _padding) ||
            videoH > canvasH * (1.0 - _padding)) {
          final scaleW = (canvasW * (1.0 - _padding)) / videoW;
          final scaleH = (canvasH * (1.0 - _padding)) / videoH;
          scale = scaleW < scaleH ? scaleW : scaleH;
        }
        ...
        final bgByteData =
            await bgImg.toByteData(format: ui.ImageByteFormat.png);
        await AppPaths.writeBytes(bgPath, bgByteData!.buffer.asUint8List());
        ...
        final maskImg =
            await maskRecorder.endRecording().toImage(scaledW, scaledH);
        final maskByteData =
            await maskImg.toByteData(format: ui.ImageByteFormat.png);
        await AppPaths.writeBytes(maskPath, maskByteData!.buffer.asUint8List());
        ...
        await AppPaths.writeBytes(
          tempFramePath,
          frameAssetData.buffer.asUint8List(),
        );
        ...
        maskCanvas.drawRRect(
          RRect.fromRectAndRadius(
            Rect.fromLTWH(
              0,
              0,
              scaledHoleW.toDouble(),
              scaledHoleH.toDouble(),
            ),
        ...
        args = [
          '-i', _videoPath,
          '-framerate', '60', '-loop', '1', '-t', durationSecs, '-i',
          tempFramePath,
          '-framerate', '60', '-loop', '1', '-t', durationSecs, '-i', maskPath,
          '-framerate', '60', '-loop', '1', '-t', durationSecs, '-i', bgPath,
          '-filter_complex',
      ...
      final durationMicros =
          _controller.value.duration.inMicroseconds.toDouble();
      ...
      if (result.success) {
        ...
        if (mounted) {
          ...
          showToast(
            context,
            detail.isEmpty
                ? 'Erro ao exportar (ffmpeg). Confirme que o ffmpeg está instalado.'
                : 'Erro ffmpeg: ${detail.length > 180 ? detail.substring(detail.length - 180) : detail}',
            isError: true,
          );
        }
      }
  ...
  Future<void> _exportVideoWeb() async {
    if (_selectedFramePath == null) return;
    ...
    try {
      final size = _computeExportCanvasSize();
      final frameless = _selectedFramePath == 'frameless';
      Uint8List? frameBytes;
      Map<String, dynamic>? bounds;
      if (!frameless && _selectedFramePath != null) {
        bounds = _framesData[_selectedFramePath!];
        final data = await rootBundle.load(_selectedFramePath!);
        frameBytes = data.buffer.asUint8List();
      }

      final layout = WebExportLayout(
        canvasW: size.$1,
        canvasH: size.$2,
        backgroundColor: _backgroundColor,
        backgroundGradient: _backgroundGradient,
        frameAssetPath: frameless ? null : _selectedFramePath,
        framePngBytes: frameBytes,
        frameBounds: bounds,
        padding: _padding,
        cropTop: _cropTop,
      ...
      final bytes = await exportComposedWebVideo(
        videoUrl: _videoPath,
        layout: layout,
        onProgress: (p) => progressNotifier.value = p,
      );
      ...
      final filename =
          'Xowcase_Export_${DateTime.now().millisecondsSinceEpoch}.webm';
      await downloadBytes(bytes, filename);
      if (mounted) {
        showToast(context, 'Exportado com device frame e fundo: $filename');
      }
    } catch (e) {
      if (mounted) {
        Navigator.of(context, rootNavigator: true).pop();
        showToast(context, 'Erro no export: $e', isError: true);
      }
    } finally {
  ...
  (int, int) _computeExportCanvasSize() {
    int canvasW = 1080;
    int canvasH = 1920;
    ...
    if (_resolution == '16:9 (YouTube)') {
      ...
      if (_selectedFramePath == 'frameless') {
        ...
        double videoH = _controller.value.size.height == 0
            ? 1080
            : _controller.value.size.height.toDouble();
        final padSafe = _padding > 0.9 ? 0.9 : _padding;
        canvasW = (videoW / (1.0 - padSafe)).toInt();
        canvasH = (videoH / (1.0 - padSafe)).toInt();
      } else {
        final bounds = _framesData[_selectedFramePath!];
        final frameW = bounds['frame_w'].toDouble();
        final frameH = bounds['frame_h'].toDouble();
        final padSafe = _padding > 0.9 ? 0.9 : _padding;
        canvasW = (frameW / (1.0 - padSafe)).toInt();
        canvasH = (frameH / (1.0 - padSafe)).toInt();
      }
      if (canvasW > 3840) {
        final capScale = 3840 / canvasW;
        canvasW = 3840;
        canvasH = (canvasH * capScale).toInt();
      }
      if (canvasW % 2 != 0) canvasW -= 1;
    ...
    // Keep web canvas sizes manageable for MediaRecorder performance.
    const maxSide = 1920;
    if (canvasW > maxSide || canvasH > maxSide) {
      final scale = maxSide / (canvasW > canvasH ? canvasW : canvasH);
      canvasW = (canvasW * scale).round();
      canvasH = (canvasH * scale).round();
      if (canvasW % 2 != 0) canvasW -= 1;
      if (canvasH % 2 != 0) canvasH -= 1;
    }
